  ...
  delay_(delay)
{
  ...
  // The three human models. The extremity model does not need an unclampable map.
  ...
  try {
    if (use_kalman_filter_) {
      ...
      if (last_meas_timestep_ != -1) {
        ...
        if (dt < 0) {
          spdlog::error("HumanReach::measurement: dt is negative. Measurement not accepted! dt = {}", dt);
          return;
        } else if (dt < 1e-7) {
